...
After forking and cloning the project, you will have to add an `.env` file and add in it the following lines:
...
Obviously, naming is a matter of preference so we used very descriptive names for route handlers and views.
...
- `occupation` - String (what the celebrity does, why they are famous. For example _actor, singer, comedian_)
...
1. In the app file (`index.js`) create the following GET route handler: `/celebrities/create`
...
4. Create the `/celebrities/create` POST route handler in `index.js`.
...
6. In the `views/home.hbs` view file:
...
1. Create the `/celebrities` GET route handler in `index.js`.
2. In the route:
...
4. In the `views/home.hbs` (homepage) file:
...
Now when we've started all this good work, let's keep up strong and build all the route handlers for the _Movie_ model. But first, let's create the _Movie_ model.
...
- Create 2 new route handlers, one to render page with the form on it, and one to send the data to after the form is filled out
...
2. Create the `/movies/:id` GET route handler in `index.js`.
3. In the route:
...
2. Create the `/movies/:id/delete` POST route in your `index.js` file
3. In the route:
...
Here are the route handlers we will be using:
...
1. Create the `/movies/:id/edit` GET route in `index.js`.
2. In that route:
...
4. Create `/movies/:id` POST route in the `index.js` file
5. In that route:
